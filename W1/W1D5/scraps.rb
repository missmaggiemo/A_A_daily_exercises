...
class KnightPathFinder
  ...
  def initialize(start_pos)
    @start = KnightMove.new(start_pos)
    @move_tree = []
  end

  def find_path(finish)

    # return path if build_move_tree.include? finish

  end

  def build_move_tree(knight_move=@start)

    knight_move.new_move_positions.each {|move| @move_tree << move unless @move_tree.include? move}




    # take finish point and look at all available moves from finish point

    # do any of those moves lead to start point?

    # if not, find_path(new_finish)
  end
